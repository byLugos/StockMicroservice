...
public interface ArticleCategoryRepository extends JpaRepository<ArticleCategoryEntity,Long> {
    @Query("SELECT ac.article.id, ac.article.name, ac.article.description, ac.article.quantity, ac.article.price, ac.category.id, ac.category.name " +
            "FROM ArticleCategoryEntity ac")
    List<Object[]> findAllArticleDetailsWithCategories();
